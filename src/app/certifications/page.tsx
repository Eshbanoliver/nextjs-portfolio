...
function CertificationCard({ cert, index }: { cert: typeof certifications[0], index: number }) {
    ...
    const showPlaceholder = isPdf || imageError;
            ...
            <div className="relative h-48 overflow-hidden bg-surface-200/30 shrink-0 group-hover:bg-surface-200/50 transition-colors">
                {showPlaceholder ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-8">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 transition-all duration-500 group-hover:scale-110" style={{ backgroundColor: `${color}10`, color: color }}>
                            {isPdf ? (
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest opacity-40 group-hover:opacity-100 transition-opacity" style={{ color: color }}>
                        ...
                        {isPdf && (
                            <a
                                ...
                                className="mt-2 text-[9px] font-bold hover:underline"
                                style={{ color: color }}
                            >
                                View Source PDF
                            </a>
                ...
                <div className="mt-auto space-y-3">
                    <div className="grid grid-cols-1 gap-2">
                        ...
                        <Button
                            ...
                            variant="outline"
                            external={isPdf}
                            className={`w-full text-[11px] py-2.5 rounded-xl ${!isPdf ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                            <HiOutlineShieldCheck className="w-4 h-4 mr-2" />
                            {isPdf ? "Verify PDF Source" : "Verify Authenticity"}
                        </Button>
                    </div>
